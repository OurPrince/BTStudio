/**
 * interceptor.js — BTStudio v1.0 | world: MAIN, run_at: document_start
 *
 * ═══════════════════════════════════════════════════════════════════
...
(function () {
  ...
  window.addEventListener('__aisu_toggle', (e) => {
    if (bypassEnabled === e.detail) return; // Prevent spam
    bypassEnabled = e.detail;
    console.log(
      `%c[BTStudio] 🛡️ Bypass ${bypassEnabled ? 'ACTIVE ✅' : 'DISABLED ❌'}`,
  ...
  XMLHttpRequest.prototype.send = function (body) {
    ...
    xhr.abort = function () {
      ...
      console.log(
        '%c[BTStudio] 🚫 abort() blocked — stream continues',
      ...
      Object.defineProperty(xhr, 'responseText', {
        get: function () {
          ...
          if (clean !== raw && !didLogSanitize) {
            didLogSanitize = true;
            console.log(
              '%c[BTStudio] ✅ Block signal neutralized — text preserved',
  ...
  console.log(
    '%c[BTStudio] ⚡ v4.0 — Angular Bypass active',
